...
loadInput :: [String] -> Int -> Int -> (M.Map (Int, Int, Int, Int) Bool) -> (M.Map (Int, Int, Int, Int) Bool)
loadInput [] _ _ m = m
...
neighbourCheck :: (Int, Int, Int, Int) -> (M.Map (Int, Int, Int, Int) Bool) -> Int
...
currentIterate :: [Int] -> [Int] -> [Int] -> [Int] -> (M.Map (Int, Int, Int, Int) Bool) -> (M.Map (Int, Int, Int, Int) Bool) -> (M.Map (Int, Int, Int, Int) Bool)
currentIterate xs ys zs ws m n_m = let range = [(x,y,z,w) | x <- xs, y <- ys, z <- zs, w <- ws] 
...
iterator :: Int -> Int -> (M.Map (Int, Int, Int, Int) Bool) -> [String] -> Int
iterator c t m r
    ...
    | otherwise  = iterator (c+1) t n_m r
        where r_x = length $ head r
              ...
              n_y_r = [(0-c)..(r_y+c)] 
              n_z_r = [(0-c)..(0+c)]
              n_m = currentIterate n_x_r n_y_r n_z_r n_z_r m M.empty
