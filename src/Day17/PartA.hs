...
type MapType = M.Map (Int, Int, Int) Bool
...
neighbourCheck :: (Int, Int, Int) -> MapType -> Int
...
currentIterate :: [Int] -> [Int] -> [Int] -> MapType -> MapType
currentIterate xs ys zs m = let range = [(x,y,z) | x <- xs, y <- ys, z <- zs] 
...
iterator c t m r
    ...
    | otherwise  = iterator (c+1) t n_m r
        where r_x = length $ head r
              ...
              n_y_r = [(0-c)..(r_y+c)] 
              n_z_r = [(0-c)..(0+c)]
              n_m = currentIterate n_x_r n_y_r n_z_r m
